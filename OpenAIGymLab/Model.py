...
class Model:
    ...
    def _define_model_2(self):
        ...
        self._q_s_a = tf.placeholder(shape=[None, self._num_actions],
                                     dtype=tf.float32)
        # create three fully connected hidden layers
        self._fc1 = tf.layers.dense(self._states, 50, activation=tf.nn.relu)
        self._fc2 = tf.layers.dense(self._fc1, 100, activation=tf.nn.relu)
        self._fc3 = tf.layers.dense(self._fc2, 200, activation=tf.nn.relu)
