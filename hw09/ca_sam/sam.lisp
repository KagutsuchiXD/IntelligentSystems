...
(defun process-cd (cd)
    "Processes cd and by integrating it into a script."
  (let ((rslt nil))
...
(defparameter *shopping-story-cds*
  '((PTRANS 
	 (:TIME (PAST)) 
...
(setf (events-script '$SHOPPING) ;;; your code here
      '((ptrans (:actor ?shopper)
				(:object ?shopper)
				(:to ?store)
				(:time ?time))
						
		(atrans (:actor ?shopper)
				(:object ?item)
				(:time ?time))
